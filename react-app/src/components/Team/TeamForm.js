    ...
    if (formType === "Create Team") {
      ...
      const returnedTeam = await dispatch(fetchCreateTeam(tempTeam))
        .then(async (team1) => {
          ...
          await dispatch(fetchOneTeam(team1.id)).then(setShowTModal(false));
  ...
  return (
    <div className=" all-margin-small col width-20em">
      ...
      <form className="" onSubmit={handleSubmit}>
        ...
        <div className="">
          <input
            type="submit"
            value={formType}
            className=" bg-white round-sq-05 thin-bor b-margin"
      ...
      {formType === "Edit Team" && (
        <div className="">
          <button
            onClick={() => deleteEvents(team.id)}
            className="bg-white round-sq-05 thin-bor"
          >
            Delete
          </button>
        </div>
