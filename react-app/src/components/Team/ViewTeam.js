...
const ViewTeam = () => {
  ...
  let oneTeamObj = useSelector((state) => {
      ...
      <div className="col lr-margin-large">
        ...
        <div className="col">
          ...
          {user && user.id == oneTeamObj.owner.id && (
            <div className=" vh-5 ai-c jc-st">
              <button
                onClick={() => (
                  setShowTModal(true), setSentTeamId(oneTeamObj.id)
                )}
        ...
        <div>
          ...
          <div className="row ai-c width-members tb-margin">
            <div
              ...
              style={{ backgroundColor: arrayOfColors[oneTeamObj.owner.id] }}
            >
              {oneTeamObj.owner.username.slice(0, 2)}
            </div>
        ...
        <div className="col t-margin jc-c">
          <div className="tb-margin">
            Members ({oneTeamObj.memberships.length})
          </div>
          <div className="long-gray-line"></div>
          <div className="row flex-wrap">
            {user &&
              !oneTeamObj.memberships.find(
                (member) => member.users[0].id == user.id
              ) && (
                <div className="row ai-c width-members tb-margin">
                  <button
                    className="no-bor bg-white jc-st ai-c cursor pad-0"
                    onClick={() => handleCreateMembership(oneTeamObj.id)}
                  >
            ...
            <div className="ai-c jc-c">
              {oneTeamObj.memberships && !oneTeamObj.memberships.length && (
                <div>This team does not yet have any members</div>
              )}
            </div>

            {oneTeamObj.memberships &&
                  ...
                  <div>
                    ...
                    <div className="row ai-c width-members tb-margin">
                      <div
                        className="solid-circle jc-c ai-c font-small-med pad-04"
                        style={{
                          backgroundColor: arrayOfColors[member.users[0].id],
                        }}
                      >
        ...
        <div className="long-gray-line"></div>
        {oneTeamObj.projects && !oneTeamObj.projects.length && (
                    ...
                    <div className="row">
                      <div className="box-circle-mem-no-margin">
                        <div
                          ...
                          style={{ backgroundColor: arrayOfColors[project.id] }}
                        >
                          <i className="fa-solid fa-list-ul"></i>
                        </div>
                      </div>

                      <div className="should-wrap-100-per lr-margin-small">
                        {project.name}{" "}
                      </div>
                    </div>
                    <div className="box-circle-mem-no-margin">
                      <div
                        className="solid-circle jc-c ai-c font-small-med pad-04"
                        style={{
                          backgroundColor: arrayOfColors[project.owner.id],
                        }}
                      >
